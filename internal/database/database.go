...
func ConnectDB() (*Database, error) {
	...
	defaultConnStr := fmt.Sprintf("postgres://%s:%s@%s:%s/postgres?sslmode=disable", user, password, host, port)
	...
	if !exists {
		...
		_, err = defaultDB.DB.Exec(fmt.Sprintf("CREATE DATABASE %s", database))
		if err != nil {
			defaultDB.Close()
